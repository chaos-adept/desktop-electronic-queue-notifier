...
function onReady() {
  ...
  socket.on('updateTopMostTicket', function (data) {
    ...
    updateActiveTicketTicks(hasTicket && data.ticket.ticks);

  })

  socket.on('onTick', function (ticks) {
    updateActiveTicketTicks(ticks);
  });
...
function updateActiveTicketTicks(ticks) {
  document.querySelector('.topTicketTicks-placeholder').classList.toggle('topTicketTicks', !!ticks);
  document.querySelector('.topTicketTicks-placeholder').classList.toggle('hidden', !ticks);
  document.querySelector('.topTicketTicks-placeholder').innerText = moment.utc(moment.duration(ticks, 'seconds').as('milliseconds')).format('mm:ss');
}
